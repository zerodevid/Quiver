...
const exact = {
  ...
  'nilainya nol, tidak bisa dipaksa': ['Position value is zero, so the forced size cannot be applied', 'Nilai posisi nol, nominal paksa tidak bisa diterapkan'],
  'batas satu sisi': ['one-sided limit', 'batas satu sisi'],
...
const rules = [
  ...
  [/^hasilnya (\$[\d.,]+) \(< minimum (\$[\d.,]+)\) dan paksa (\$[\d.,]+) tidak muat$/,
    'Size {1} is below the {2} minimum and the forced {3} does not fit',
    'Ukuran {1} di bawah minimum {2} dan nominal paksa {3} tidak muat'],
  [/^(.+?) tinggal (\$[\d.,]+)$/s, '{1} is down to {2}', '{1} tinggal {2}'],
