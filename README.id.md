...
<p align="center">
  <a href="docs/quiver-demo-en.mp4">
    <img src="docs/demo-poster.jpg" alt="Video demo Quiver — 3:07, 1080p" width="800" />
  </a><br />
  <sub>Tur dasbor bernarasi (3:07, narasi Inggris). Alamat wallet, nama target, dan nomor posisi disensor; semua nilai dikalikan faktor rahasia. <a href="docs/quiver-demo-en.srt">Subtitle</a></sub>
</p>
...
**Panen fee otomatis (v3 & v4):** tombol Panen fee di posisi dasbor dan Telegram. Atur ON/OFF, mode, minimum nilai fee, dan interval pemeriksaan sendiri. Default OFF, mode compound, $5, 30 menit. Bot berjalan hanya saat LIVE dan tidak dijeda; slippage dan batas posisi/eksposur mengikuti Aturan. Jadwalnya milik sendiri — **bukan** mengikuti kapan target memanen fee-nya (lihat “Sinyal panen target”).

- **Mode compound** — fee dikembalikan jadi likuiditas di posisi yang sama, tanpa swap. v4 memakai satu batch INCREASE + TAKE_PAIR; v3 memakai multicall `collect` + `increaseLiquidity` pada NonfungiblePositionManager (perlu izin ERC20 ke NPM, dikirim sekali). Token fee yang tidak cocok dengan rasio LP masuk ke wallet. Siklus dengan fee yang bisa dipasangkan di bawah minimum dilewati. Hasil compound tetap keuntungan, bukan tambahan modal dari luar.
- **Mode klaim** — fee ditarik ke wallet. Sisi aset kuotasi (USDG/ETH) langsung jadi uang; sisi memecoin-nya masuk antrean jual yang sama dengan sisa penutupan posisi dan ditukar ke aset kuotasi pool lewat Kyber, dengan batas rugi rute yang sama (saklar “jual sisi memecoin” bisa dimatikan). Jumlah yang dijual dibaca dari receipt klaim, bukan saldo wallet.

**Pembukuan fee yang belum terjual:** klaim membukukan kedua sisi ke `claimed_quote` di harga pool saat klaim. Sisi memecoin-nya dicatat di tabel `fee_leftovers` sampai benar-benar terjual, lalu taksiran itu **diganti hasil jual sesungguhnya** — persis pola `left_token`/`left_quote` untuk sisa penutupan, hanya saja kolom yang dikoreksi `claimed_quote` (dan `out_quote` kalau posisinya sudah ditutup). Satu penjualan yang menyentuh dua buku dibagi FIFO, fee lebih dulu (`Positions.recordTokenSale`), termasuk untuk swap manual dari halaman Swap. Memecoin fee yang hilang dari wallet di luar bot (dijual di DEX lain, dikirim keluar) ketahuan saat sinkron: kekurangannya dianggap terealisasi di harga kini, sama seperti perlakuan untuk sisa penutupan.

**Sinyal panen target:** panen fee target (ModifyLiquidity v4 dengan delta likuiditas nol) dicatat sebagai aksi `claim` dan **tidak pernah dicermin** — klaim bukan aksi pasar, jadi menirunya cuma membakar gas. Yang dipakai polanya: kalau target memanen ≥ 3× dalam 24 jam pada posisi yang cerminnya kita pegang, satu kabar dikirim (didiamkan 6 jam sesudahnya) karena panen beruntun sering mendahului keluarnya target. Klaim v3 target belum dipantau: event `Collect` NPM tidak ikut dalam query log yang sekarang, dan menambahkannya menggandakan volume log yang sudah mepet kuota.
