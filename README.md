...
- **Auto-compound (v4):** use the **Auto-compound** button on a position in the dashboard or Telegram. Set ON/OFF, minimum fees to reinvest, and the check interval yourself. Defaults are OFF, $5 and 30 minutes; automation runs only in LIVE mode while unpaused. A fixed liquidity increase uses accrued fees atomically (`INCREASE_LIQUIDITY` + `TAKE_PAIR`), with no wallet deposit or swap. Unmatched fee tokens go to the wallet; cycles with insufficient usable fees are skipped. Slippage and position/exposure caps follow Rules. Reinvestment remains profit rather than new contributed capital, and pending transactions are reconciled after restart.
