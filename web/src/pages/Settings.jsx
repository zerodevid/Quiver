...
const amt = (v, d = 4) => (v == null ? '—' : Number(v).toLocaleString('id-ID', { maximumFractionDigits: d }));

function Section({ title, desc, children }) {
  return (
    <div className="flex flex-col gap-5">
      <div><h2 className="text-lg font-semibold">{title}</h2>{desc && <p className="mt-1 text-sm text-muted">{desc}</p>}</div>
      {children}
    </div>
  );
}
const say = (r, ok) => (r.error ? toast.danger(r.error) : toast.success(ok));
...
function WalletTab({ d, reload }) {
  ...
  return (
    <Section title="Wallet bot" desc={<>Pakai wallet khusus bot, jangan wallet utama. Kunci privat disimpan di server (<span className="mono">{w.keyFile}</span>) dan tidak pernah ditampilkan lagi.</>}>
      {w.address ? (
        <dl className="grid gap-x-8 gap-y-4 sm:grid-cols-3">
          <div className="sm:col-span-2"><dt className="text-xs text-muted">Alamat</dt><dd className="mono mt-0.5 break-all">{w.address}</dd></div>
          <div><dt className="text-xs text-muted">Izin berkas kunci</dt><dd className="mt-1">
            <Chip size="sm" variant="soft" color={w.perms === '600' ? 'success' : 'danger'}>{w.perms === '600' ? '600 · aman' : `${w.perms || '?'} · terlalu longgar`}</Chip></dd></div>
      ...
      ) : <Notice>Belum ada wallet terpasang. Bot hanya bisa berjalan dalam mode simulasi.</Notice>}

      <Separator />
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="font-medium">Mode: {m.dry_run ? 'Simulasi' : 'Live'}</div>
          <div className="text-sm text-muted">{m.dry_run ? 'Bot memutuskan dan mencatat, tapi tidak mengirim transaksi.' : 'Bot mengirim transaksi sungguhan dari wallet di atas.'}</div>
        </div>
        {m.dry_run ? (
          <div className="flex items-end gap-2">
            ...
            <Button variant="danger" isDisabled={!w.address || liveTxt !== 'LIVE'} isPending={busy === 'live'}
              onPress={() => act('live', '/api/settings/live', { live: true, confirm: liveTxt }, 'Mode LIVE menyala')}>Nyalakan LIVE</Button>
          </div>
        ) : (
          <Button variant="outline" isPending={busy === 'live'} onPress={() => act('live', '/api/settings/live', { live: false }, 'Kembali ke simulasi')}>Kembali ke simulasi</Button>
        )}
      </div>

      <Separator />
      {!m.dry_run ? <Notice status="warning">Matikan mode LIVE dulu untuk mengganti wallet.</Notice> : (
        <>
          <div className="grid gap-6 md:grid-cols-2">
            <div className="flex flex-col gap-3">
              <div className="font-medium">Impor kunci privat</div>
              ...
              <Button variant="outline" className="w-fit" isDisabled={!pk} isPending={busy === 'imp'}
                onPress={async () => { const r = await act('imp', '/api/settings/wallet/import', { privateKey: pk, replace }, (x) => `Wallet ${x.address} terpasang`); if (!r.error) setPk(''); }}>
                <KeyRound className="size-4" />Impor</Button>
            </div>
            <div className="flex flex-col gap-3">
              <div className="font-medium">Buat wallet baru</div>
              <p className="text-sm text-muted">Kunci dibuat di server. Frasa pemulihan disimpan di sebelah berkas kunci.</p>
              <Button variant="outline" className="w-fit" isPending={busy === 'gen'}
                onPress={() => act('gen', '/api/settings/wallet/generate', { replace }, (x) => `Wallet baru ${x.address} dibuat`)}>Buat wallet</Button>
            </div>
          </div>
          <Checkbox isSelected={replace} onChange={setReplace}>
            <Checkbox.Content><Checkbox.Control><Checkbox.Indicator /></Checkbox.Control>
              Ganti kunci yang sudah ada — kunci lama dipindah ke berkas cadangan, tidak dihapus</Checkbox.Content>
          </Checkbox>
          {w.address && (
            <div className="flex flex-col gap-3 rounded-md border border-border p-4">
              <div className="font-medium">Lepas wallet</div>
              <div className="flex flex-wrap items-end gap-2">
                ...
                <Button variant="danger" isDisabled={rm.toLowerCase() !== w.address} isPending={busy === 'rm'}
                  onPress={() => act('rm', '/api/settings/wallet/remove', { confirm: rm }, 'Wallet dilepas, kunci dicadangkan')}>Lepas</Button>
              </div>
            </div>
          )}
        </>
...
function Caps({ e }) {
  return (
    <div className="flex flex-wrap gap-1">
      {e.no_logs ? <Chip size="sm" variant="soft">tanpa getLogs</Chip>
        : e.max_log_blocks ? <Chip size="sm" variant="soft" color="warning">getLogs ≤ {num(e.max_log_blocks)} blok</Chip>
        : <Chip size="sm" variant="soft" color="success">getLogs penuh</Chip>}
      {e.archive && <Chip size="sm" variant="soft" color="accent">arsip</Chip>}
      {e.secret && <Chip size="sm" variant="soft"><KeyRound className="size-3" />API key</Chip>}
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3 py-4">
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="num w-40 shrink-0 text-right text-sm">
          ...
          <div className="whitespace-nowrap text-xs text-muted">{num(e.calls)} panggilan · {e.errors} err</div>
          {e.cooling && <div className="text-xs text-warning">istirahat</div>}
        </div>
        <div className="flex gap-1">
          <Button size="sm" variant={edit ? 'secondary' : 'ghost'} isIconOnly aria-label="Ubah" onPress={() => setEdit(!edit)}><Pencil className="size-4" /></Button>
          <Button size="sm" variant="ghost" isIconOnly aria-label="Uji" isPending={testing} onPress={runTest}><Pulse className="size-4" /></Button>
          <Button size="sm" variant="ghost" isIconOnly aria-label="Hapus" onPress={() => onDelete(e.id)}><Trash2 className="size-4 text-danger" /></Button>
        </div>
      </div>
      {edit && (
        <div className="grid items-end gap-4 rounded-md bg-surface-secondary p-4 md:grid-cols-4">
          ...
          <Button onPress={() => { onSave(f); setEdit(false); }}>Simpan</Button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <Section title="Endpoint RPC" desc="Permintaan dibagi otomatis: getLogs hanya ke endpoint yang sanggup, pembacaan state lampau hanya ke endpoint arsip, sisanya ke yang paling senggang. Perubahan berlaku tanpa restart.">
      <div className="divide-y divide-border rounded-md border border-border px-4">
        {d.rpc.map((e) => (
          <RpcRow key={e.id + e.url} e={e}
            ...
            onDelete={(id) => { if (confirm('Hapus endpoint ini?')) saveList(current().filter((x) => x.id !== id), 'Endpoint dihapus'); }} />
        ))}
      </div>

      <Separator />
      <div className="font-medium">Tambah endpoint</div>
      ...
      {tested && (tested.bad
        ...
        : <Notice status="success" title={tested.summary}><span className="mr-2">Bendera yang akan dipasang:</span><span className="inline-flex align-middle"><Caps e={{ ...tested, secret: !!tested.headers }} /></span></Notice>)}
      <div className="flex gap-2">
        <Button variant="outline" isDisabled={!url.trim()} isPending={testing} onPress={runTest}>Uji dulu</Button>
        <Button isDisabled={!tested || tested.bad} onPress={add}>Tambah</Button>
      </div>
    </Section>
  );
}
...
function SimpleForm({ title, desc, fields, initial, url, okText, extra }) {
  ...
  return (
    <Section title={title} desc={desc}>
      ...
      <div className="flex gap-2"><Button onPress={save} isPending={busy}>Simpan</Button>{extra}</div>
    </Section>
  );
}
...
function SecurityTab() {
  ...
  return (
    <Section title="Keamanan" desc="Dasbor ini bisa menyalakan LIVE dan menutup posisi, jadi dilindungi token akses.">
      <div>
        <div className="font-medium">Ganti token akses</div>
        <p className="mb-3 mt-1 text-sm text-muted">Token lama langsung tidak berlaku; perangkat lain harus masuk ulang. Browser ini tetap masuk.</p>
        <Button variant="danger" onPress={rotate}>Buat token baru</Button>
      </div>
      {tok && (
        <Card variant="secondary"><Card.Content className="gap-2">
          <div className="text-sm font-medium">Token baru — simpan sekarang, tidak akan ditampilkan lagi</div>
          <div className="flex items-center gap-2"><code className="mono flex-1 break-all rounded bg-surface px-3 py-2">{tok}</code>
            <Button variant="outline" onPress={() => { navigator.clipboard?.writeText(tok); toast.success('Tersalin'); }}><Copy className="size-4" />Salin</Button></div>
        </Card.Content></Card>
      )}
    </Section>
  );
              ...
              <Tabs.ListContainer className="md:w-48 md:shrink-0">
                <Tabs.List aria-label="Bagian pengaturan">
                  {[['wallet', 'Wallet & mode'], ['rpc', 'RPC'], ['gas', 'Gas'], ['notify', 'Notifikasi'], ['loop', 'Mesin'], ['security', 'Keamanan']].map(([id, label]) => (
                    <Tabs.Tab key={id} id={id} className="justify-start">{label}<Tabs.Indicator /></Tabs.Tab>
                  ))}
                </Tabs.List>
              </Tabs.ListContainer>
              <div className="min-w-0 flex-1">
                ...
                <Tabs.Panel id="notify">
                  <SimpleForm title="Notifikasi" url="/api/settings/notify" okText="Topik tersimpan" initial={d.notify}
                    ...
                    extra={<Button variant="outline" onPress={async () => say(await post('/api/settings/notify/test', {}), 'Notifikasi uji terkirim')}>Kirim uji</Button>} />
                </Tabs.Panel>
                <Tabs.Panel id="loop">
