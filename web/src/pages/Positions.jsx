  ...
  return (
    <>
      ...
      entrySqrt={p.entrySqrt} exitSqrt={p.exitSqrt} />) },
            { key: 'val', label: 'Nilai', align: 'end', render: (p) => (<div>{usd(p.valueUsd)}<div className="text-xs text-muted">{t('modal {v}', { v: usd(p.costUsd) })}</div></div>) },
            ...
            { key: 'act', label: '', render: (p) => <Button size="sm" variant="danger" onPress={() => close(p.id)}>{t('Tutup')}</Button> },
          ]} />
      </Panel>
