    ...
    <>
      ...
      <Panel className="mb-6">
        <div className="grid items-end gap-3 md:grid-cols-[1fr_auto]">
          ...
          <Button onPress={() => open()} isDisabled={!valid}>Buka</Button>
        </div>
        {recent.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <span className="text-sm text-muted">Pernah dipindai:</span>
            {recent.map((w) => (
              <Button key={w.address} size="sm" variant={w.address === current ? 'secondary' : 'outline'} onPress={() => open(w.address)}>
                ...
                <span className="text-muted">· {w.positions_n} posisi ·</span>
